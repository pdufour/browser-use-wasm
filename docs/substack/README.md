...
| `dpr-rounding-demo.mjs` | Fractional CSS × DPR live vs capture on a text card |
| `dpr-rounding-demo.html` | Standalone page for the fractional-pixel demo |
...
| DPR fractional pixels | `browser-use-dpr-rounding-substack.png` |
...
## DPR fractional pixels

Live vs SnapDOM on a normal text card with a fractional CSS height (`316.3px`). Shows how `× devicePixelRatio` yields a non-integer device-pixel backing size.

```bash
npm run dev
# http://127.0.0.1:5173/substack/
